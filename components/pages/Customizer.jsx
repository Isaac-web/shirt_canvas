                ...
                <div>
                    <motion.div className="fixed left-1  top-1/3 md:top-1/4" {...slideAnimation("left")}>
                        <div className="flex flex-col gap-5 bg-red-300 p-2 rounded-md glassmorphism">
                            {EditorTabs.map(tab =>
                                <button key={tab.name} className="tab-btn" onClick={() => handleSurfaceCustomizer(tab)}>
                                    <Image alt={tab.name} src={tab.icon} width={42} height={42} />
                                </button>
                            )}
                        </div>
                            <motion.div {...fadeAnimation} className="z-50">
                                {activePicker === "colorpicker" && <div className="absolute top-0 left-20 glassmorphism rounded-md flex flex-col px-3 py-2">
                                    <div className="flex flex-row justify-end">
                                        <button className="p-1" style={{ color: snap.color }} onClick={() => setActivePicker(null)}>
                                ...
                                {activePicker === "filepicker" && <div className="picker-container w-60  flex flex-col justify-between">
                                    <FilePicker />
                                </div>}
                            </motion.div>
                    </motion.div>
                    ...
                    <motion.div className="fixed bottom-0 w-full flex justify-center gap-x-12" {...slideAnimation('up')}>
                        <div className="p-5">
                            {FilterTabs.map(tab => (
                                <button key={tab.name} className="mx-2" onClick={() => setActiveButtomTab(tab.name)}>
